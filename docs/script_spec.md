 ...
 # NeuroScript: A script for humans, AIs and computers (Revised 2025-04-30)
 ...
 * **Set Statement:** Assigns the result of an expression to a variable. Variables are dynamically typed and scoped to the procedure execution.
     ...
     set result = tool.ReadFile("input.txt") # Calls are expressions
     set sum = 1 + 2
     ```
 ...
 * **Procedure/Tool Calls as Statements:** A call expression can stand alone as a statement if its return value is intentionally ignored.
     ```neuroscript
     tool.LogMessage("Starting phase 1") # Return value ignored
     MyProcedureWithoutReturnValues()   # Return value ignored (or is nil)
     ```
 ...
 * **Calls (`callable_expr`):**
     ...
     * Tools: `tool.ReadFile("path/to/file")`
 ...
 * **Parentheses:** `(1 + 2) * 3` for grouping.
 ...
 External capabilities are exposed via tools, prefixed with `tool.`.
 ```neuroscript
 set content = tool.ReadFile("config.json")
 set status = tool.WriteFile("output.log", log_data)
 ```
 Tools are registered within the interpreter and handle interactions with the file system, network, external processes, etc. Standard tools (availability may vary) include `ReadFile`, `WriteFile`, `ListFiles`, `ExecuteCommand`, `GoBuild`, `GoCheck`, `GoFmt`, `VectorIndex`, `VectorSearch`, `GitAdd`, `GitCommit`, etc.
 ...
 * `must`/`mustbe` check conditions and trigger a specific `ErrMustConditionFailed` if false.
 ...
 func ReadAndGreet(needs file_path) returns status means
   ...
   set file_content = tool.ReadFile(file_path)
   must file_content != nil # Basic check if read succeeded (tool might return nil on error)
 ...
 * **Syntax:** Uses `func`/`endfunc`, optional `needs`/`optional`/`returns` clauses (parens optional), `means`, `if`/`endif`, `while`/`endwhile`, `for each`/`endfor`, `on_error`/`endon`. Calls (`MyProc()`, `tool.MyTool()`) are expressions.
 ...
 * **Current Implementation:** Core parsing and execution working for G4 syntax. Basic tools available. Metadata parsing/storage implemented. `ask` statement implemented.
