...
import (
	...
	"github.com/aprice2704/neuroscript/pkg/core" // For AIWorkerInstance, ConversationTurn
...
type ChatSession struct {
	...
	Conversation   []*core.ConversationTurn // The history of this specific chat session
...
func NewChatSession(sessionID, displayName, definitionID string, instance *core.AIWorkerInstance) *ChatSession {
	...
	return &ChatSession{
		...
		Conversation:   make([]*core.ConversationTurn, 0), // Initialize with an empty history
		CreatedAt:      time.Now(),
...
func (cs *ChatSession) AddTurn(turn *core.ConversationTurn) {
	if cs == nil {
...
func (cs *ChatSession) GetConversationHistory() []*core.ConversationTurn {
	if cs == nil {
		return []*core.ConversationTurn{} // Return an empty slice if cs is nil
	}
	cs.mu.RLock()
	defer cs.mu.RUnlock()

	// Create a copy of the slice to prevent external modifications to the slice itself (e.g., reordering).
	// The ConversationTurn pointers are copied.
	historyCopy := make([]*core.ConversationTurn, len(cs.Conversation))
...
func (cs *ChatSession) ClearConversation() {
	...
	cs.mu.Lock()
	defer cs.mu.Unlock()
	cs.Conversation = make([]*core.ConversationTurn, 0) // Replace with a new empty slice
	cs.LastActivityAt = time.Now()
}
