...
func toolFileHash(interpreter tool.Runtime, args []interface{}) (interface{}, error) {
	...
	if sandboxRoot == "" {
		interpreter.GetLogger().Error("Tool: FileHash] Interpreter sandboxDir is empty, cannot proceed.")
	...
	if openErr != nil {
		...
		if errors.Is(openErr, os.ErrPermission) {
			...
			interpreter.GetLogger().Warn("Tool: FileHash] %s", errMsg)
		...
		interpreter.GetLogger().Error("Tool: FileHash] %s: %v", errMsg, openErr)
	...
	if statErr != nil {
		...
		interpreter.GetLogger().Error("Tool: FileHash] %s: %v", errMsg, statErr)
	...
	if copyErr != nil {
		...
		interpreter.GetLogger().Error("Tool: FileHash] %s: %v", errMsg, copyErr)
