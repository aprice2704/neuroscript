...
func toolFileHash(interpreter *Interpreter, args []interface{}) (interface{}, error) {
	// --- Argument Validation ---
	if len(args) != 1 {
		return "", lang.NewRuntimeError(ErrorCodeArgMismatch, fmt.Sprintf("FileHash: expected 1 argument (filepath), got %d", len(args)), ErrArgumentMismatch)
	}
	filePathRel, ok := args[0].(string)
	if !ok {
		return "", lang.NewRuntimeError(ErrorCodeType, fmt.Sprintf("FileHash: filepath argument must be a string, got %T", args[0]), ErrInvalidArgument)
	}
	if filePathRel == "" {
		return "", lang.NewRuntimeError(ErrorCodeArgMismatch, "FileHash: filepath cannot be empty", ErrInvalidArgument)
	}
	...
	if sandboxRoot == "" {
		...
		return "", lang.NewRuntimeError(ErrorCodeConfiguration, "FileHash: interpreter sandbox directory is not set", ErrConfiguration)
	}
	...
	if openErr != nil {
		...
		if errors.Is(openErr, os.ErrNotExist) {
			...
			return "", lang.NewRuntimeError(ErrorCodeFileNotFound, errMsg, ErrFileNotFound)
		}
		if errors.Is(openErr, os.ErrPermission) {
			...
			return "", lang.NewRuntimeError(ErrorCodePermissionDenied, errMsg, ErrPermissionDenied)
		}
		// Other open errors
	...
	if statErr != nil {
		...
		return "", lang.NewRuntimeError(ErrorCodeIOFailed, errMsg, errors.Join(ErrIOFailed, statErr))
	}
	if stat.IsDir() {
		...
		return "", lang.NewRuntimeError(ErrorCodePathTypeMismatch, errMsg, ErrPathNotFile)
	}
	...
	if copyErr != nil {
		...
		return "", lang.NewRuntimeError(ErrorCodeIOFailed, errMsg, errors.Join(ErrIOFailed, copyErr))
	}
