...
func toolTreeSetNodeMetadata(interpreter *Interpreter, args []interface{}) (interface{}, error) {
	...
	if !okMetaKey {
		return nil, lang.NewRuntimeError(ErrorCodeType, fmt.Sprintf("%s: metadata_key argument must be a string, got %T", toolName, args[2]), ErrInvalidArgument)
	}
	metaValue, okMetaValue := args[3].(string)
	if !okMetaValue {
		return nil, lang.NewRuntimeError(ErrorCodeType, fmt.Sprintf("%s: metadata_value argument must be a string, got %T", toolName, args[3]), ErrInvalidArgument)
	}

	if metaKey == "" {
		return nil, lang.NewRuntimeError(ErrorCodeArgMismatch, fmt.Sprintf("%s: metadata_key cannot be empty", toolName), ErrInvalidArgument)
	}
