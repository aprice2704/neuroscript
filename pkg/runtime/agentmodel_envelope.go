...
// File version: 1
// Purpose: AgentModel envelope and runtime validation against ExecPolicy grants for safe 'ask' execution.
// filename: pkg/runtime/agentmodel_envelope.go
// nlines: 127
// risk_rating: MEDIUM
...
func (p *ExecPolicy) ValidateAgentModelEnvelope(env AgentModelEnvelope) error {
	if p == nil {
		return fmt.Errorf("policy: %w", ErrPolicy)
	}
	...
	if !cap.CapsSatisfied(needs, p.Grants.Grants) {
		return fmt.Errorf("policy: %w (missing env/net/model grants for %q)", ErrCapability, env.Name)
	}
	...
	if c := strings.TrimSpace(env.BudgetCurrency); c != "" {
		...
		if env.MinPerCallCents > 0 {
			...
			if pc > 0 && pc < env.MinPerCallCents {
				return fmt.Errorf("policy: %w (per-call budget %s %d < required %d)", ErrCapability, c, pc, env.MinPerCallCents)
			}
		}
		// Per-run
		if env.MinPerRunCents > 0 {
			...
			if rc > 0 && rc < env.MinPerRunCents {
				return fmt.Errorf("policy: %w (per-run budget %s %d < required %d)", ErrCapability, c, rc, env.MinPerRunCents)
			}
		}
	}
