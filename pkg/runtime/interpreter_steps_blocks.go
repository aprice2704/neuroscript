...
func (i *Interpreter) executeIf(step ast.Step, isInHandler bool, activeError *RuntimeError) (result Value, wasReturn bool, wasCleared bool, err error) {
	posStr := "<unknown_pos>"
	if step.Pos != nil {
	...
	if step.Cond == nil {
		return nil, false, false, lang.NewRuntimeError(ErrorCodeInternal, "IF step has nil Condition", nil).WithPosition(step.Pos)
	}
...
func (i *Interpreter) executeWhile(step ast.Step, isInHandler bool, activeError *RuntimeError) (result Value, wasReturn bool, wasCleared bool, err error) {
	posStr := "<unknown_pos>"
	if step.Pos != nil {
		posStr = step.Pos.String()
	}
	...
	if step.Cond == nil {
		return nil, false, false, lang.NewRuntimeError(ErrorCodeInternal, "WHILE step has nil Condition", nil).WithPosition(step.Pos)
	}
	...
	for {
		...
		if iteration > maxIterations {
			...
			return nil, false, wasCleared, lang.NewRuntimeError(ErrorCodeInternal, errMsg, errors.New("max iterations exceeded")).WithPosition(step.Pos)
		}
...
func (i *Interpreter) executeFor(step ast.Step, isInHandler bool, activeError *RuntimeError) (result Value, wasReturn bool, wasCleared bool, err error) {
	posStr := "<unknown_pos>"
	if step.Pos != nil {
		posStr = step.Pos.String()
	}
	...
	if collectionExpr == nil {
		return nil, false, false, lang.NewRuntimeError(ErrorCodeInternal, "FOR EACH step has nil Collection expression", nil).WithPosition(step.Pos)
	}
	if loopVar == "" {
		return nil, false, false, lang.NewRuntimeError(ErrorCodeInternal, "FOR EACH step has empty LoopVarName", nil).WithPosition(step.Pos)
	}
	...
	handleIteration := func(item Value) (bool, bool, error) {
		...
		if iteration > maxIterations {
			...
			return false, false, lang.NewRuntimeError(ErrorCodeInternal, errMsg, errors.New("max iterations exceeded")).WithPosition(step.Pos)
		}
		if setErr := i.SetVariable(loopVar, item); setErr != nil {
			errMsg := fmt.Sprintf("setting loop variable '%s' in FOR EACH", loopVar)
			return false, false, lang.NewRuntimeError(ErrorCodeInternal, errMsg, setErr).WithPosition(step.Pos)
		}
	...
	default:
		...
		return nil, false, wasCleared, lang.NewRuntimeError(ErrorCodeType, errMsg, nil).WithPosition(collectionExpr.GetPos())
	}
