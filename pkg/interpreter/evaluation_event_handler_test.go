...
// File version: 10
// Purpose: Moved test into the 'interpreter' package to resolve access issues with unexported functions and types.
// nlines: 135+
// risk_rating: LOW
...
// FIX: Now part of the interpreter package, so it can use unexported members.
func setupEventHandlerTest(t *testing.T, script string) (*Interpreter, error) {
	t.Helper()

	logger := logging.NewTestLogger(t)

	// FIX: Use the exported NewInterpreter with functional options.
	...
	// FIX: Correctly call the NewASTBuilder method from the parser API.
...
func TestOnEventHandling(t *testing.T) {
	...
	t.Run("Multiple handlers for the same event", func(t *testing.T) {
		...
		// FIX: Use the exported test helper for dumping variables.
