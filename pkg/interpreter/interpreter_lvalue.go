...
func (i *Interpreter) setSingleLValue(lvalueExpr ast.Expression, rhsValue lang.Value) error {
	...
	// If the variable doesn't exist, is nil, or is NOT a container,
	// we must build the entire required structure from scratch.
	...
	// The variable exists and is a container, so we can safely modify it.
	// We clone it first to prevent partial modification on error.
...
// It can assume `current` is always a container (*MapValue or *ListValue).
func (i *Interpreter) traverseAndSet(current lang.Value, accessors []*ast.AccessorNode, valueToPlace lang.Value) error {
	...
	// This should be unreachable if the logic in setSingleLValue is correct.
