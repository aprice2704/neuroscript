...
func SecureFilePath(filePath, allowedDir string) (string, error) {
	// Basic validation for input filePath
	if filePath == "" {
	...
	// Join the allowed directory with the relative input path and clean it
	// This resolves 'filePath' relative to 'absAllowedDir'
	absCleanedPath := filepath.Join(absAllowedDir, filePath)
	absCleanedPath = filepath.Clean(absCleanedPath)

	// Check if the cleaned, absolute path is within the allowed directory
	// Add a separator to absAllowedDir for prefix checking if it's not the root "/"
	// This handles cases where absAllowedDir might be "/foo" and absCleanedPath "/foobar"
	...
	// The path is safe if it's exactly the allowed directory OR it starts with the allowed directory path followed by a separator.
