...
func toolInput(interpreter *Interpreter, args []interface{}) (interface{}, error) {
	...
	if len(args) > 0 {
		if p, ok := args[0].(string); ok {
			...
			return "", NewRuntimeError(ErrorCodeType, fmt.Sprintf("Input: prompt argument must be a string or null, got %T", args[0]), ErrInvalidArgument)
		}
	}
	...
	if err != nil {
		...
		return "", NewRuntimeError(ErrorCodeIOFailed, "failed to read input", errors.Join(ErrIOFailed, err))
	}
...
func toolPrint(interpreter *Interpreter, args []interface{}) (interface{}, error) {
	...
	if len(args) != 1 {
		...
		return nil, NewRuntimeError(ErrorCodeArgMismatch, "Print: tool implementation expected exactly 1 argument ('values')", ErrArgumentMismatch)
	}
