...
func toolLoadScript(interpreter *Interpreter, args []interface{}) (interface{}, error) {
	if len(args) != 1 {
		return nil, NewRuntimeError(ErrorCodeArgMismatch, "LoadScript requires exactly one argument: script_content (string)", nil)
	}
	scriptContent, ok := args[0].(string)
	if !ok {
		return nil, NewRuntimeError(ErrorCodeArgMismatch, "LoadScript argument must be a string", nil)
	}
	if scriptContent == "" {
		return nil, NewRuntimeError(ErrorCodeArgMismatch, "LoadScript argument 'script_content' cannot be an empty string", nil)
	}
	...
	if parseErr != nil {
		return nil, NewRuntimeError(ErrorCodeSyntax, fmt.Sprintf("parsing script failed: %s", parseErr.Error()), parseErr)
	}
	if parseTree == nil {
		return nil, NewRuntimeError(ErrorCodeInternal, "internal error: parser returned nil ANTLR tree", nil)
	}
	...
	if buildErr != nil {
		return nil, NewRuntimeError(ErrorCodeSyntax, fmt.Sprintf("building AST for script failed: %s", buildErr.Error()), buildErr)
	}
	if programAST == nil {
		return nil, NewRuntimeError(ErrorCodeInternal, "internal error: AST builder yielded a nil Program", nil)
	}

	// Stage 3: Load into Interpreter
	if err := interpreter.LoadProgram(programAST); err != nil {
		return nil, NewRuntimeError(ErrorCodeExecutionFailed, fmt.Sprintf("failed to load program into interpreter: %s", err.Error()), err)
	}
