...
func debugCalculateCanonicalPath(modulePath, moduleRootDir, dirPath string, logger *log.Logger) (string, error) {
	logPrefix := "[DEBUG PATH CALC]"
	if logger == nil {
		logger = log.New(io.Discard, "", 0) // Prevent nil pointer panic
	}
	logger.Printf("%s Inputs: modulePath=%q, moduleRootDir=%q, dirPath=%q", logPrefix, modulePath, moduleRootDir, dirPath)

	// --- Input Validation ---
	if moduleRootDir == "" {
		err := errors.New("moduleRootDir cannot be empty")
		logger.Printf("%s FAIL: %v", logPrefix, err)
		return "", err
	}
	if modulePath == "" {
	...
	if relErr != nil {
		logger.Printf("%s Base FAIL: filepath.Rel(%q, %q) error: %v", logPrefix, moduleRootDir, dirPath, relErr)
	...
	logger.Printf("%s Method 1 Result (RelFromRoot -> ToSlash -> path.Clean): %q", logPrefix, m1Path)
	...
	logger.Printf("%s Method 2 Result (path.Join(module, RelFromRootSlash)): %q", logPrefix, m2Path)
	...
	logger.Printf("%s Method 3 Result (String Concat): %q", logPrefix, m3Path)
	...
	if relFromModuleRootSlash == "." {
		...
		logger.Printf("%s Final Logic: Path is module root, using modulePath: %q", logPrefix, canonicalPath)
	} else {
		...
		if strings.HasPrefix(m2Path, doublePrefix) {
			...
			logger.Printf("%s Final Logic: Method 2 result %q detected doubling, using Method 1 result: %q", logPrefix, m2Path, canonicalPath)
		} else {
			// Otherwise, Method 2 (path.Join) is the standard, correct way.
			canonicalPath = m2Path
			logger.Printf("%s Final Logic: Method 2 result %q seems correct, using it.", logPrefix, canonicalPath)
		}
	}

	// Final clean just in case (should be redundant if m1Path/m2Path are clean)
	canonicalPath = path.Clean(canonicalPath)
	logger.Printf("%s Final Logic Returning Path: %q", logPrefix, canonicalPath)
