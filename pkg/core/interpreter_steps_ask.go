...
func (i *Interpreter) executeAskAI(step Step) error {
	...
	if !ok {
		return NewRuntimeError(ErrorCodeInternal, fmt.Sprintf("Internal error at %s: 'ask' step value is not an Expression (got %T)", step.Pos.String(), step.Value), nil)
	}
	...
	if i.llmClient == nil {
		return NewRuntimeError(ErrorCodeLLMError, fmt.Sprintf("LLM client is not configured in the interpreter (required at %s)", step.Pos.String()), nil)
	}
	...
	if err != nil {
		return NewRuntimeError(ErrorCodeLLMError, fmt.Sprintf("LLM interaction failed (ask at %s)", step.Pos.String()), err)
	}
	if responseTurn == nil {
		return NewRuntimeError(ErrorCodeLLMError, fmt.Sprintf("LLM returned nil response without error (ask at %s)", step.Pos.String()), nil)
	}
...
func (i *Interpreter) handleToolCalls(calls []*interfaces.ToolCall, pos *Position) error {
	i.logger.Debug("Handling tool calls requested by LLM", "count", len(calls), "pos", pos.String())
	if len(calls) == 0 {
		return nil
	}
	if i.toolRegistry == nil {
		return NewRuntimeError(ErrorCodeConfiguration, fmt.Sprintf("cannot handle tool calls at %s: toolRegistry is nil", pos.String()), nil)
	}
