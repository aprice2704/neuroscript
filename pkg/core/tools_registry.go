...
func (r *ToolRegistryImpl) CallFromInterpreter(interp *Interpreter, toolName string, args []Value) (Value, error) {
	...
	if len(rawArgs) < len(impl.Spec.Args) {
		...
		return nil, NewRuntimeError(ErrorCodeArgMismatch, fmt.Sprintf("tool '%s': expected at least %d args, got %d", toolName, len(impl.Spec.Args), len(rawArgs)), ErrArgumentMismatch)
	}
	...
	for i, spec := range impl.Spec.Args {
		...
		if err != nil {
			return nil, NewRuntimeError(ErrorCodeArgMismatch, fmt.Sprintf("tool '%s' arg '%s': %v", toolName, spec.Name, err), ErrArgumentMismatch)
		}
	}
