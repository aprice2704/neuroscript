...
// File version: 0.1.2 // Utilized existing coreNoOpLogger from pkg/core/utils.go.
...
type StructuredSyntaxError struct {
	...
	Column          int    // 0-based character position in line from ANTLR
...
func (l *ErrorListener) SyntaxError(recognizer antlr.Recognizer, offendingSymbol interface{}, line, column int, msg string, e antlr.RecognitionException) {
	...
	finalColumn := column // ANTLR column is 0-based character position in line
	finalMsg := msg
	offendingTokenText := ""
...
func (p *ParserAPI) Parse(source string) (antlr.Tree, error) {
	...
	lexerErrorListener := NewErrorListener(p.logger) // Uses coreNoOpLogger if p.logger is nil due to NewParserAPI
	lexer.AddErrorListener(lexerErrorListener)

	stream := antlr.NewCommonTokenStream(lexer, antlr.TokenDefaultChannel)
	parser := gen.NewNeuroScriptParser(stream)
	parser.RemoveErrorListeners()
	parserErrorListener := NewErrorListener(p.logger) // Uses coreNoOpLogger if p.logger is nil
...
func (p *ParserAPI) ParseForLSP(sourceName string, sourceContent string) (antlr.Tree, []StructuredSyntaxError) {
	...
	lexer := gen.NewNeuroScriptLexer(inputStream)
	lexer.RemoveErrorListeners()
	errorListener := NewLSPErrorListener(sourceName, p.logger) // Uses coreNoOpLogger if p.logger is nil
