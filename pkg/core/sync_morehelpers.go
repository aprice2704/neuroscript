...
// It now returns the initialized/defaulted loggers.
func initializeSyncState(infoLog, errorLog, debugLog *log.Logger) (
	stats map[string]interface{},
	incrementStat func(string),
	iLog, eLog, dLog *log.Logger, // Return modified loggers
) {
	// Assign defaults if input is nil
	iLog = infoLog
	if iLog == nil {
		iLog = log.New(io.Discard, "INFO: ", log.LstdFlags)
	}
	eLog = errorLog
	if eLog == nil {
		eLog = log.New(io.Discard, "ERROR: ", log.LstdFlags|log.Lshortfile)
	}
	dLog = debugLog
	if dLog == nil {
		dLog = log.New(io.Discard, "DEBUG: ", log.LstdFlags|log.Lshortfile)
	}
	...
	// Use the potentially defaulted eLog in the closure
	incrementStat = func(key string) {
		if v, ok := stats[key].(int64); ok {
			stats[key] = v + 1
		} else {
			eLog.Printf("[CRITICAL SYNC HELPER] Invalid stat key %s", key)
		} // Use eLog here
	}
	// Return the (potentially defaulted) loggers along with stats and function
	return stats, incrementStat, iLog, eLog, dLog
}
...
func listExistingAPIFiles(sc *syncContext) (map[string]*genai.File, error) {
	...
	sc.infoLog.Println("[API HELPER List] Fetching file list from API...") // Use context's infoLog
	if sc.ctx == nil {
	...
	for {
		...
		if err != nil {
			...
			sc.errorLog.Printf("[API HELPER List] %s", errMsg) // Use context's errorLog
			fetchErrors++
			continue
		}
...
func initializeGitignore(sc *syncContext, ignoreGitignore bool) *gitignore.GitIgnore {
	...
	if sc.infoLog == nil || sc.errorLog == nil || sc.debugLog == nil {
		log.Println("ERROR: initializeGitignore called with nil loggers in syncContext!")
		return nil // Cannot proceed safely
	}
	if ignoreGitignore {
		sc.infoLog.Println("[API HELPER Sync] Ignoring .gitignore")
		return nil
	}
	gitignorePath := filepath.Join(sc.absLocalDir, ".gitignore")
	sc.debugLog.Printf("[DEBUG API HELPER Sync] Loading gitignore: %s", gitignorePath)
	ignorer, gitignoreErr := gitignore.CompileIgnoreFile(gitignorePath)
	if gitignoreErr != nil {
		if os.IsNotExist(gitignoreErr) {
			sc.infoLog.Println("[API HELPER Sync] No .gitignore found.")
		} else {
			sc.errorLog.Printf("[WARN API HELPER Sync] Error read gitignore %s: %v", gitignorePath, gitignoreErr)
		}
		return nil
	}
	if ignorer != nil {
