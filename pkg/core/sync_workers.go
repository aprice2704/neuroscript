...
func startUploadWorkers(sc *syncContext, wg *sync.WaitGroup, actions SyncActions, resultsChan chan<- uploadResult) {
	...
	if totalUploadJobs == 0 {
		sc.infoLog.Println("[API HELPER Sync] No upload/update jobs required.")
		return
	}
	const maxConcurrentUploads = 8
	jobsChan := make(chan uploadJob, totalUploadJobs)

	sc.debugLog.Printf("[DEBUG API HELPER Sync] Starting %d upload workers for %d jobs...", maxConcurrentUploads, totalUploadJobs)
	for i := 0; i < maxConcurrentUploads; i++ {
		...
		go func(workerID int) {
			...
			for job := range jobsChan {
				sc.debugLog.Printf("[API HELPER Sync Worker %d] STARTING job for: %s (Update: %t)", workerID, job.relPath, job.existingApiFile != nil)
				apiFile, uploadErr := processUploadJob(sc, job, workerID) // Pass workerID
				sc.debugLog.Printf("[API HELPER Sync Worker %d] processUploadJob finished for: %s (Error: %v)", workerID, job.relPath, uploadErr)
				sc.debugLog.Printf("[API HELPER Sync Worker %d] Sending result to resultsChan for: %s", workerID, job.relPath)
				resultsChan <- uploadResult{job: job, apiFile: apiFile, err: uploadErr}
				sc.debugLog.Printf("[API HELPER Sync Worker %d] FINISHED job for: %s", workerID, job.relPath)
			}
	...
	sc.debugLog.Printf("[DEBUG API HELPER Sync] Queuing %d upload jobs...", len(actions.FilesToUpload))
	...
	sc.debugLog.Printf("[DEBUG API HELPER Sync] Queuing %d update jobs...", len(actions.FilesToUpdate))
	for _, updateJob := range actions.FilesToUpdate {
		jobsChan <- updateJob
	}
	sc.debugLog.Println("[DEBUG API HELPER Sync] Finished queuing jobs.")
	close(jobsChan)
}
...
func processUploadJob(sc *syncContext, job uploadJob, workerID int) (*genai.File, error) {
	sc.debugLog.Printf("[DEBUG processUploadJob %d] Entered for: %s", workerID, job.relPath)
	defer sc.debugLog.Printf("[DEBUG processUploadJob %d] Exiting for: %s", workerID, job.relPath)
	var uploadErr error
	var apiFile *genai.File

	if job.existingApiFile != nil { // Pre-delete logic
		sc.debugLog.Printf("[API HELPER Sync Worker %d] Deleting existing %s for update: %s", workerID, job.existingApiFile.Name, job.relPath)
		...
		cancelDel()
		if deleteErr != nil {
			sc.errorLog.Printf("[ERROR Worker %d] Pre-delete fail %s (%s): %v", workerID, job.existingApiFile.Name, job.relPath, deleteErr)
		} else {
			sc.debugLog.Printf("[API Worker %d] Pre-delete OK %s (%s)", workerID, job.existingApiFile.Name, job.relPath)
		}
	...
	sc.infoLog.Printf("[API Worker %d] %s: %s...", workerID, operation, job.relPath) // Log start of operation

	sc.debugLog.Printf("[DEBUG processUploadJob %d] Calling HelperUploadAndPollFile: %s", workerID, job.relPath)
	...
	cancelUpload()
	sc.debugLog.Printf("[DEBUG processUploadJob %d] HelperUploadAndPollFile returned: %s (Err: %v)", workerID, job.relPath, uploadErr)
...
func startDeleteWorkers(sc *syncContext, wg *sync.WaitGroup, filesToDelete []*genai.File) {
	if len(filesToDelete) == 0 {
		sc.infoLog.Println("[API HELPER Sync] No delete jobs required.")
		return
	}
	const maxConcurrentDeletes = 16
	deleteJobsChan := make(chan *genai.File, len(filesToDelete))
	sc.debugLog.Printf("[DEBUG API HELPER Sync] Starting %d delete workers for %d jobs...", maxConcurrentDeletes, len(filesToDelete))
	...
	for i := 0; i < maxConcurrentDeletes; i++ {
		...
		go func(workerID int) {
			...
			for fileToDelete := range deleteJobsChan {
				...
				// sc.infoLog.Printf("[API Delete Worker %d] Deleting: %s (%s)...", workerID, displayName, fileToDelete.Name)
				...
				if deleteErr != nil {
					...
					sc.errorLog.Printf("[ERROR Delete Worker %d] Fail delete %s (%s): %v", workerID, fileToDelete.Name, displayName, deleteErr)
				} else {
					sc.incrementStat("files_deleted_api")
					sc.debugLog.Printf("[API Delete Worker %d] Deleted OK: %s (%s)", workerID, fileToDelete.Name, displayName)
				}
				time.Sleep(50 * time.Millisecond)
			}
			sc.debugLog.Printf("[API HELPER Sync Delete Worker %d] Exiting.", workerID)
		}(i)
	}
	sc.debugLog.Printf("[DEBUG API HELPER Sync] %d delete workers started.", maxConcurrentDeletes)
	sc.debugLog.Printf("[DEBUG API HELPER Sync] Queuing %d delete jobs...", len(filesToDelete))
	for _, file := range filesToDelete {
		deleteJobsChan <- file
	}
	close(deleteJobsChan)
	sc.debugLog.Println("[DEBUG API HELPER Sync] Finished queuing delete jobs.")
}
