...
// File version: 10
// Purpose: ADDED PRINTF DEBUGGING to ExitError_handler to diagnose test failures.
...
func (l *neuroScriptListenerImpl) EnterError_handler(c *gen.Error_handlerContext) {
	// The generic enterBlockContext is handled by EnterStatement_list for the handler's body.
}

// ExitError_handler is called when exiting an error handler block.
// It now attaches the handler to the current procedure's dedicated ErrorHandlers list.
func (l *neuroScriptListenerImpl) ExitError_handler(c *gen.Error_handlerContext) {
	// --- FORCED DEBUGGING VIA PRINTF ---
	fmt.Printf("\n>>>> DEBUG | ENTERING ExitError_handler <<<<\n")
	if l.currentProc != nil {
		fmt.Printf(">>>> DEBUG | currentProc context is SET. Name: '%s'\n", l.currentProc.Name)
	} else {
		fmt.Printf(">>>> DEBUG | currentProc context is NIL. Handler will not be attached.\n")
	}
	// --- END DEBUGGING ---
	...
	// Pop the sentinel that was pushed by the grammar rule for the 'on' statement
	marker, ok := l.popValue()
	if !ok || marker != onEventMarker { // Assumes 'onEventMarker' is a known constant
	...
	// Create the handler step object.
	handlerStep := &Step{
		Pos:  tokenToPosition(parentCtx.KW_ON().GetSymbol()),
		Type: "on_error_handler", // Using a more specific type for clarity
		Body: body,
	}

	// This attaches the handler to the procedure's dedicated slice.
	if l.currentProc != nil {
		l.currentProc.ErrorHandlers = append(l.currentProc.ErrorHandlers, handlerStep)
		// --- FORCED DEBUGGING VIA PRINTF ---
		fmt.Printf(">>>> DEBUG | Appended handler to l.currentProc.ErrorHandlers. New count: %d <<<<\n\n", len(l.currentProc.ErrorHandlers))
		// --- END DEBUGGING ---
	} else {
		l.addError(c, "'on error' is only allowed inside a procedure")
	}
}
...
func (l *neuroScriptListenerImpl) EnterClearEventStmt(c *gen.ClearEventStmtContext) {}
func (l *neuroScriptListenerImpl) ExitClearEventStmt(c *gen.ClearEventStmtContext)  {}
