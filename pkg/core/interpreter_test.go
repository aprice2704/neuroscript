...
func runExecuteStepsTest(t *testing.T, tc executeStepsTestCase) {
	t.Helper()
	interp := newTestInterpreter(nil, nil) // Start with nil last result unless specified by test?
	if tc.initialVars != nil {
		interp.variables = make(map[string]interface{}, len(tc.initialVars))
		for k, v := range tc.initialVars {
			interp.variables[k] = v
		}
		// If initialVars contains a LAST value, set it? No, usually set by CALL.
	}
...
func TestExecuteStepsBlocksAndLoops(t *testing.T) {
	testCases := []executeStepsTestCase{
		...
		{name: "IF true literal", inputSteps: []Step{createIfStep(BooleanLiteralNode{Value: true}, []Step{createTestStep("SET", "x", nil, StringLiteralNode{Value: "Inside"}, nil)}, nil)}, initialVars: map[string]interface{}{}, expectedVars: map[string]interface{}{"x": "Inside"}, expectedResult: nil, expectError: false},                                                                               // Used createIfStep helper
		{name: "IF false literal", inputSteps: []Step{createIfStep(BooleanLiteralNode{Value: false}, []Step{createTestStep("SET", "x", nil, StringLiteralNode{Value: "Inside"}, nil)}, nil), createTestStep("SET", "y", nil, StringLiteralNode{Value: "Outside"}, nil)}, initialVars: map[string]interface{}{}, expectedVars: map[string]interface{}{"y": "Outside"}, expectedResult: nil, expectError: false}, // Used createIfStep helper
		{name: "IF condition var true", inputSteps: []Step{createIfStep(VariableNode{Name: "cond_var"}, []Step{createTestStep("SET", "x", nil, StringLiteralNode{Value: "Inside"}, nil)}, nil)}, initialVars: map[string]interface{}{"cond_var": true}, expectedVars: map[string]interface{}{"cond_var": true, "x": "Inside"}, expectedResult: nil, expectError: false},
		{name: "IF block with RETURN", inputSteps: []Step{createTestStep("SET", "status", nil, StringLiteralNode{Value: "Started"}, nil), createIfStep(BooleanLiteralNode{Value: true}, []Step{createTestStep("SET", "x", nil, StringLiteralNode{Value: "Inside"}, nil), createTestStep("RETURN", "", nil, StringLiteralNode{Value: "ReturnedFromIf"}, nil), createTestStep("SET", "y", nil, StringLiteralNode{Value: "NotReached"}, nil)}, nil), createTestStep("SET", "status", nil, StringLiteralNode{Value: "Finished"}, nil)}, initialVars: map[string]interface{}{}, expectedVars: map[string]interface{}{"status": "Started", "x": "Inside"}, expectedResult: "ReturnedFromIf", expectError: false},
		...
		// --- Tool Call Tests ---
		// *** UPDATED: Use LastNode{} instead of LastCallResultNode{} ***
		{name: "CALL TOOL StringLength AST", inputSteps: []Step{createTestStep("SET", "myStr", nil, StringLiteralNode{Value: "Test"}, nil), createTestStep("CALL", "TOOL.StringLength", nil, nil, []interface{}{VariableNode{Name: "myStr"}}), createTestStep("SET", "lenResult", nil, LastNode{}, nil)}, initialVars: map[string]interface{}{}, expectedVars: map[string]interface{}{"myStr": "Test", "lenResult": int64(4)}, expectedResult: nil, expectError: false},
		{name: "CALL TOOL Substring AST", inputSteps: []Step{createTestStep("CALL", "TOOL.Substring", nil, nil, []interface{}{StringLiteralNode{Value: "ABCDE"}, NumberLiteralNode{Value: int64(1)}, NumberLiteralNode{Value: int64(4)}}), createTestStep("SET", "sub", nil, LastNode{}, nil)}, initialVars: map[string]interface{}{}, expectedVars: map[string]interface{}{"sub": "BCD"}, expectedResult: nil, expectError: false},
