...
func toolDeleteFile(interpreter *Interpreter, args []interface{}) (interface{}, error) {
	if len(args) != 1 {
		return nil, NewRuntimeError(ErrorCodeArgMismatch, fmt.Sprintf("DeleteFile: expected 1 argument (path), got %d", len(args)), ErrArgumentMismatch)
	}
	relPath, ok := args[0].(string)
	if !ok {
		return nil, NewRuntimeError(ErrorCodeType, fmt.Sprintf("DeleteFile: path argument must be a string, got %T", args[0]), ErrInvalidArgument)
	}
	if relPath == "" {
	...
	if sandboxRoot == "" {
		...
		return nil, NewRuntimeError(ErrorCodeConfiguration, "DeleteFile: interpreter sandbox directory is not set", ErrConfiguration)
	}
	...
	if err != nil {
		...
		if isDirNotEmptyErr {
			...
			return nil, NewRuntimeError(ErrorCodePreconditionFailed, errMsg+": directory not empty", errors.Join(ErrCannotDelete, err))
		}
