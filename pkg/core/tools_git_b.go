...
func toolGitClone(interpreter *Interpreter, args []interface{}) (interface{}, error) {
	...
	if _, err := os.Stat(absTargetPath); err == nil {
		return nil, NewRuntimeError(ErrorCodePathExists, fmt.Sprintf("Git.Clone: target path '%s' already exists", relativePath), ErrPathExists)
	} else if !os.IsNotExist(err) {
		return nil, NewRuntimeError(ErrorCodeIOFailed, fmt.Sprintf("Git.Clone: error checking target path '%s'", relativePath), errors.Join(ErrIOFailed, err))
	}
