...
func toolFileHash(interpreter *Interpreter, args []interface{}) (interface{}, error) {
	...
	if sandboxRoot == "" {
		if interpreter.logger != nil {
			interpreter.logger.Printf("[WARN TOOL FileHash] Interpreter sandboxDir is empty, using default relative path validation.")
		}
	...
	if secErr != nil {
		...
		if interpreter.logger != nil {
			interpreter.logger.Printf("[TOOL FileHash] %s (Sandbox Root: %s)", errMsg, sandboxRoot)
		}
	...
	// --- File Hashing ---
	if interpreter.logger != nil {
		interpreter.logger.Printf("[TOOL FileHash] Attempting to hash validated path: %s (Original Relative: %s, Sandbox: %s)", absPath, filePathRel, sandboxRoot)
	}
	...
	if err != nil {
		...
		if interpreter.logger != nil {
			interpreter.logger.Printf("[TOOL FileHash] %s", errMsg)
		}
	...
	if interpreter.logger != nil {
		interpreter.logger.Printf("[TOOL FileHash] Successfully calculated SHA256 hash for %s: %s", filePathRel, hashString)
	}
