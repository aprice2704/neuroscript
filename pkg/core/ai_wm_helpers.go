...
func (m *AIWorkerManager) GetDefinitionIDByName(name string) (string, error) {
	if name == "" {
		return "", NewRuntimeError(ErrorCodeArgMismatch, "worker definition name cannot be empty", ErrInvalidArgument)
	}
	...
	return "", NewRuntimeError(ErrorCodeKeyNotFound, fmt.Sprintf("worker definition name '%s' not found", name), ErrNotFound)
}
...
func (m *AIWorkerManager) GetDefinition(definitionID string) (*AIWorkerDefinition, error) {
	if definitionID == "" {
		return nil, NewRuntimeError(ErrorCodeArgMismatch, "worker definition ID cannot be empty", ErrInvalidArgument)
	}
	...
	if !exists || def == nil {
		// Corrected: ErrorCodeItemNotFound -> ErrorCodeKeyNotFound, ErrItemNotFound -> ErrNotFound
		return nil, NewRuntimeError(ErrorCodeKeyNotFound, fmt.Sprintf("worker definition ID '%s' not found", definitionID), ErrNotFound)
	}

	if def.Status != DefinitionStatusActive {
		// Corrected: ErrItemNotFound -> ErrConfiguration (to match ErrorCodeConfiguration)
		return nil, NewRuntimeError(ErrorCodeConfiguration, fmt.Sprintf("worker definition '%s' (ID: %s) is not active (status: %s)", def.Name, def.DefinitionID, def.Status), ErrConfiguration)
	}
	return def, nil
}
