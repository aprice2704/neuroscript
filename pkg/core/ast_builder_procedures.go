...
import (
	// Removed strings import - no longer needed here
...
func (l *neuroScriptListenerImpl) EnterProcedure_definition(ctx *gen.Procedure_definitionContext) {
	...
	l.currentProc = &Procedure{
		...
		// Docstring field removed
	}

	// Metadata (::) is skipped by the lexer. Old COMMENT: block logic is gone.
...
func (l *neuroScriptListenerImpl) ExitProcedure_definition(ctx *gen.Procedure_definitionContext) {
	...
	if l.currentProc != nil {
		l.logDebugAST("    Appending procedure: %s (ReqParams: %d, OptParams: %d, Returns: %d, Steps: %d)", l.currentProc.Name, len(l.currentProc.RequiredParams), len(l.currentProc.OptionalParams), len(l.currentProc.ReturnVarNames), len(l.currentProc.Steps))
