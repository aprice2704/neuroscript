...
func (m *AIWorkerManager) ExecuteStatelessTask(	//
	...
	// 	return "", nil, lang.NewRuntimeError(ErrorCodePreconditionFailed, fmt.Sprintf("worker definition '%s' is not active (status: %s), cannot execute stateless task", definitionNameIn, defStatusCopy), ErrFailedPrecondition)
	// }
