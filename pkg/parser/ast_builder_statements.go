...
func (l *neuroScriptListenerImpl) ExitEmit_statement(c *gen.Emit_statementContext) {
	...
	if c.Expression() != nil {
		...
		expr, castOk := rawExpr.(ast.Expression)
		if !castOk {
			l.addError(c, "internal error: value on stack for emit was not an ast.Expression, but %T", rawExpr)
			l.push(rawExpr)
			return
		}
...
func (l *neuroScriptListenerImpl) ExitReturn_statement(c *gen.Return_statementContext) {
	...
	if exprListCtx := c.Expression_list(); exprListCtx != nil {
		...
		if numExprs > 0 {
			...
			values = make([]ast.Expression, numExprs)
			for i, rawExpr := range rawExprs {
				expr, castOk := rawExpr.(ast.Expression)
				if !castOk {
					l.addError(c, "internal error: value on stack for return statement was not an ast.Expression, but %T", rawExpr)
					l.push(rawExpr)
					return
				}
				values[i] = expr
...
func (l *neuroScriptListenerImpl) ExitFail_statement(c *gen.Fail_statementContext) {
	...
	if c.Expression() != nil {
		...
		if !ok {
			l.addError(c, "fail statement has invalid expression")
			return
		}
		expr, ok := val.(ast.Expression)
		if !ok {
			l.addError(c, "fail statement expression is not an ast.Expression, but %T", val)
			return
		}
		values = []ast.Expression{expr}
	}
...
func (l *neuroScriptListenerImpl) ExitMust_statement(c *gen.Must_statementContext) {
	...
	expr, ok := val.(ast.Expression)
	if !ok {
		l.addError(c, "must statement expression is not an ast.Expression, but %T", val)
		return
	}
...
func (l *neuroScriptListenerImpl) ExitAsk_stmt(c *gen.Ask_stmtContext) {
	...
	prompt, ok := l.poplang.Value()
	if !ok {
		l.addError(c, "ask statement requires a valid prompt expression")
		return
	}

	promptExpr, ok := prompt.(ast.Expression)
	if !ok {
		l.addError(c, "ask statement prompt is not an ast.Expression, but %T", prompt)
		return
	}
